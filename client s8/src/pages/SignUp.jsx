...
export default function SignUpPage() {
    ...
    const handleFormSubmit = async () => {
        try {
            window.scrollTo(0, 0);
            // Determine form data and endpoint based on user type
            const formData = userType === "User" ? userFormValues : bankOfficerFormValues;
            ...
            // Open OTP popup upon successful registration
            setShowOtpPopup(true);
        } catch (error) {
            console.error("Error submitting form:", error);
            // Optionally, add further error handling or display an error message
        }
    };
    ...
    const formInputs = isSignIn 
        ? signInInputs 
        : (userType === "User" ? userInputs : bankOfficerSteps[currentStep].inputs);
                            ...
                            <div className="mt-8 space-y-4">
                                <button
                                    onClick={isSignIn 
                                        ...
                                        : userType === "Bank Officer" 
                                            ? handleNextStep 
                                            : validateForm ? handleFormSubmit : null}
...
//     // Add separate handlers for sign in and sign up
//     const handleSignIn = async () => {
//         if (validateForm()) {
//             try {
//                 const signInData = userType === "User" ? userFormValues : bankOfficerFormValues;
//                 const endpoint = userType === "User" 
...
//                 // Direct navigation without OTP for sign in
//                 navigate(userType === "User" ? "/usersideprime" : "/bankofficerdashboard");
//             } catch (error) {
...
//                     const endpoint = serverUrl + "/api/v1/user/register";
//                     const response = await axios.post(endpoint, formData);
//                     console.log("Registration successful:", response.data);
                    
//                     // Show OTP popup only for registration
//                     setShowOtpPopup(true);
//                 } else {
//                     // Bank Officer registration
//                     const endpoint = serverUrl + "/api/v1/bank-user/register";
//                     const response = await axios.post(endpoint, bankOfficerFormValues);
//                     console.log("Registration successful:", response.data);
//                     setShowOtpPopup(true);
//                 }
//             } catch (error) {
...
//             // Additional validation for Bank Officer fields if needed
//             if (userType === "Bank Officer") {
//                 const currentStepInputs = bankOfficerSteps[currentStep].inputs;
//                 currentStepInputs.forEach(input => {
//                     const fieldName = input.label.toLowerCase().replace(/\s+/g, '');
//                     if (!formData[fieldName] || formData[fieldName].trim() === '') {
//                         errors[fieldName] = `${input.label} field cannot be empty`;
//                         isValid = false;
//                     }
//                 });
//             }
...
//                             <div className="mt-8 space-y-4">
//                                 <button
//                                     onClick={isSignIn ? handleSignIn : userType === "Bank Officer" ? handleNextStep : handleSignUp}
//                                     className="w-full py-3 bg-[#004663] text-white rounded-lg font-semibold hover:bg-sky-900 transition-colors duration-200"
//                                 >
//                                     {isSignIn 
//                                         ? "Sign In"
